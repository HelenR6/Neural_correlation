...
for model_type in model_type_list:
  ...
  random_indices = np.random.choice(1000, size=640, replace=False)
  IN_image_np=np.array([np.array(preprocess(Image.open(image_filepath).copy())) for image_filepath in filepaths])
  temp_IN_image_np=IN_image_np[random_indices,:]
  IN_image_tensor=torch.tensor(temp_IN_image_np)
  from PIL import Image
