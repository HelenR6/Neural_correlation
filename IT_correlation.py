...
for model_type in model_type_list:
  ...
  if not os.path.exists(f'/content/gdrive/MyDrive/IT/{domain_name}/pls_IN_pca_{model_type}_natural_neuron_corr.npy'):
      print("not exists")
#     continue
      resnet,preprocess=load_model(model_type)
      IN_image_tensor=torch.tensor(np.array([np.array(preprocess(Image.open(image_filepath).copy())) for image_filepath in filepaths]))
      from PIL import Image
      domain_path=args.domain.replace('_','/')
      final_path=domain_path[:-1]+'_'+domain_path[-1:]
      f = h5py.File('/content/gdrive/MyDrive/npc_IT_data.h5','r')
      ...
      if neuro_wise == 'True':
        if (args.base_model):
          with open(f'/content/gdrive/MyDrive/IT/{domain_name}/{args.base_model}_natural_mean.json') as json_file:
            layerlist=[]
          ...
          with open(f'/content/gdrive/MyDrive/IT/{domain_name}/{model_type}_natural_mean.json') as json_file:
            layerlist=[]
            load_data = json.load(json_file)
            json_acceptable_string = load_data.replace("'", "\"")
            d = json.loads(json_acceptable_string)
        ...
        with h5py.File(f'{args.neuro_wise}_{model_type}_synth_layer_activation.hdf5','r')as s:
            with h5py.File(f'{args.neuro_wise}_{model_type}_natural_layer_activation.hdf5','r')as f:
                ...
                #             clf = Ridge(random_state=seed)
                    ...
                    print(cc)
                    if neuro_wise=='True':
                        np.save(f'gdrive/MyDrive/IT/{domain_name}/pls_IN_pca_{model_type}_synth_neuron_corr.npy',total_synth_corr)
                        np.save(f'gdrive/MyDrive/IT/{domain_name}/pls_IN_pca_{model_type}_natural_neuron_corr.npy',total_natural_corr)
                        ...
                        with open(f"gdrive/MyDrive/IT/{domain_name}/pls_{model_type}_natural.json", 'w') as f:
                            json.dump(natural_json, f)
                        with open(f"gdrive/MyDrive/IT/{domain_name}/pls_{model_type}_synth.json", 'w') as f:
                        ...
                        print(json_object)
                        with open(f"gdrive/MyDrive/IT/{domain_name}/pls_{model_type}_natural_mean.json", 'w') as f:
                            json.dump(json_object, f)

                        json_object = json.dumps(synth_mean_dict, indent = 4) 
                        print(json_object)
                        with open(f"gdrive/MyDrive/IT/{domain_name}/pls_{model_type}_synth_mean.json", 'w') as f:
                            json.dump(json_object, f)
