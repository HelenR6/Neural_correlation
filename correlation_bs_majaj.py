...
for model_type in model_type_list:
  ...
  majaj = h5py.File('/content/gdrive/MyDrive/majaj_640.h5','r')
  majaj_data= majaj['images'][:]
  IN_image_tensor=torch.tensor(np.array([np.array(preprocess((Image.fromarray(i)).convert('RGB'))) for i in majaj_data]))
        ...
        with h5py.File(f'{args.neuro_wise}_{model_type}_natural_layer_activation.hdf5','r')as f:
            for seed in random_list:
                for k in layerlist:
                    ...
                    pca.fit(torch.tensor(c).cpu().detach().reshape(640,-1))
            ...
            #             clf = Ridge(random_state=seed)
                ...
                print(cc)
                if neuro_wise=='True':
                    np.save(f'gdrive/MyDrive/V4/{session_name}/pls_majaj_pca_640_{model_type}_synth_neuron_corr.npy',total_synth_corr)
                    np.save(f'gdrive/MyDrive/V4/{session_name}/pls_majaj_pca_640_{model_type}_natural_neuron_corr.npy',total_natural_corr)
