    ...
    with h5py.File(f'{model_type}_natural_layer_activation.hdf5','r')as f:
      for seed in random_list:
        for k in layerlist:
          ...
          # target:natural_neuron. neuron_target: synth_neuron
          hybrid_neuron=np.vstack((target,neuron_target))
          for fold, (train_ids,test_ids) in enumerate(kfold.split(synth_x_pca)):
            clf = Ridge(random_state=seed)
            clf.fit(synth_x_pca[train_ids],neuron_target[train_ids])
            synth_prediction[test_ids]=clf.predict((synth_x_pca)[test_ids])
          ...
          #     synth_prediction[synth_test_ids]=clf.predict((synth_x_pca)[synth_test_ids])
            if fold==0:
              natural_prediction=clf.predict((natural_x_pca))
            else:
              natural_prediction=natural_prediction+clf.predict((natural_x_pca))
            if fold==4:
              natural_prediction=natural_prediction/5
      ...
      print(cc)
      if neuro_wise=='True':
        np.save(f'gdrive/MyDrive/V4/{session_name}/hybrid/{model_type}_synth_neuron_corr.npy',total_synth_corr)
        np.save(f'gdrive/MyDrive/V4/{session_name}/hybrid/{model_type}_natural_neuron_corr.npy',total_natural_corr)
        ...
        with open(f"gdrive/MyDrive/V4/{session_name}/hybrid/{model_type}_natural.json", 'w') as f:
          json.dump(natural_json, f)
        with open(f"gdrive/MyDrive/V4/{session_name}/hybrid/{model_type}_synth.json", 'w') as f:
        ...
        print(json_object)
        with open(f"gdrive/MyDrive/V4/{session_name}/hybrid/{model_type}_natural_mean.json", 'w') as f:
          json.dump(json_object, f)

        json_object = json.dumps(synth_mean_dict, indent = 4) 
        print(json_object)
        with open(f"gdrive/MyDrive/V4/{session_name}/hybrid/{model_type}_synth_mean.json", 'w') as f:
          json.dump(json_object, f)
